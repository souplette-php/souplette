...
final class InCell extends RuleSet
{
    ...
    {
        $type = $token::KIND;
        if ($type === TokenKind::EndTag && ($token->name === 'td' || $token->name === 'th')) {
        ...
        } else if ($type === TokenKind::StartTag && isset(self::CLOSE_CELL_START_TAGS[$token->name])) {
            ...
            // Ignore the token.
            return;
        } else if ($type === TokenKind::EndTag && isset(self::CLOSE_CELL_END_TAGS[$token->name])) {
            // If the stack of open elements does not have an element in table scope
            // that is an HTML element with the same tag name as that of the token,
            // then this is a parse error; ignore the token.
            if (!$tree->openElements->hasTagInTableScope($token->name)) {
